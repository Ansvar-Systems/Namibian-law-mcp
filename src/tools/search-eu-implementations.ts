/**
 * search_eu_implementations — Search EU directives/regulations that have Namibian implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_namibian_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as namibian_statute_count
  ...
  if (input.has_namibian_implementation) {
    sql += ' HAVING namibian_statute_count > 0';
  }

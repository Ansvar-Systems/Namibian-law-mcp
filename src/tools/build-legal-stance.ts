...
export async function buildLegalStance(
  ...
  for (const ftsQuery of queryVariants) {
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        const deduped = annotateCitations(deduplicateResults(rows, limit));
        return {
          results: deduped,
          _meta: {
            ...generateResponseMetadata(db),
            disclaimer: 'RESEARCH ONLY — not legal advice. Verify all citations against official Namibian sources before use.',
  ...
  // LIKE fallback — last resort when all FTS5 variants return empty
  {
    ...
    try {
      ...
      if (rows.length > 0) {
        return {
          results: annotateCitations(deduplicateResults(rows, limit)),
          _meta: {
            ...generateResponseMetadata(db),
            disclaimer: 'RESEARCH ONLY — not legal advice. Verify all citations against official Namibian sources before use.',
...
/**
 * Annotate each result with a per-item _citation for entity linking.
 */
function annotateCitations(rows: LegalStanceResult[]): LegalStanceResult[] {
  return rows.map(row => ({
    ...row,

#!/usr/bin/env tsx
/**
 * Namibian Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://namiblii.org';
const PORTAL_NAME = 'Namibia Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/namibian-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Namibian Law MCP — Data Freshness Check');

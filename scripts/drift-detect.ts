#!/usr/bin/env tsx
/**
 * Drift detection for Namibia Law MCP.
 *
 * Checks if upstream Namibia Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Namibia Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'namibian-law-mcp/1.0 drift-detect' },
      });

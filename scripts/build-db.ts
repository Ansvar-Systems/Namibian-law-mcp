#!/usr/bin/env tsx
/**
 * Database builder for Namibia Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Namibia Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Namibian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'NA');
    insertMeta.run('source', 'namiblii.org');

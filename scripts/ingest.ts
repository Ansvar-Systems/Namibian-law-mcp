#!/usr/bin/env tsx
/**
 * Namibia Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from namiblii.org (Akoma Ntoso HTML).
 *
 * Features:
 ...
 *
 * Data source: namiblii.org (National Council for Law Reporting)
...
import { parseNamibiaLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Namibia Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('====================================================\n');
  console.log(`  Source: namiblii.org (National Council for Law Reporting)`);
  ...
  for (const law of acts) {
    ...
    try {
      ...
      const parsed = parseNamibiaLawHtml(html, act);
  ...
  console.log(`\n  Source:      namiblii.org (Akoma Ntoso HTML)`);
